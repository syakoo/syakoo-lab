...
これまで TypeScript + React で開発を続けてきました。その中で芽生えた小さな思想を、思い出せる範囲で言語化してみます。
...
特に、定数として CONSTANT_CASE を使用するパターンもありますが、変数は基本的に const であるため、 camelCase との境界が曖昧になってしまいます。そのため、変数は camelCase で統一します。
...
ファイルを開いたときに、その内容をすぐに把握できるようにするためです。
...
条件が並列している場合は、それぞれの条件が独立していることを明確にする必要があると考えているためです。
そのときはコードとしては冗長にはなるが、必要な情報であると考えています。
...
個人的に、コミットは細かすぎても良いとは思っていなく、その変更がプロダクトに残す情報なのかどうかとその影響するスコープで判断しています。
たとえば、フォーマットの修正はもちろん、ある機能を実現するための部分的な機能追加は、それだけでコミットを分けなくて良いと考えています。
...
このときに co-author でコミットすることで、他のメンバーとの会話の結果であることがわかり、情報が鮮明になると考えています。
