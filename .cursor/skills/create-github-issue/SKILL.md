...
| 項目 | ルール |
|------|--------|
| タイトル | テンプレの `title` フィールドの定義（プレフィックス等）に従う |
...
- **CLI**（`gh`）: `gh issue create` では `--title` と `--body`（または `-b`）に草案を渡せる。タイトルはテンプレの `title` の定義に従う。シェルで複数行を安全に渡す必要があるときだけ `--body-file` を使う。
